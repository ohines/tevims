...
RESULTS_DIR <- "Output/"
INVESTIGATION_NAME <- "actg175_gam"
...
wrapper_gam <- function(y_train, x_train, x_new) {
    SuperLearner::SL.gam(y_train, x_train, x_new, family = gaussian(), obsWeights = rep_along(y_train, 1))
}

fitfunc_outcome <- function(y_train, x_train, x_new) {
    t_learner(y_train, x_train, x_new, "A", wrapper_gam)
}
fitfunc_cate <- wrapper_gam
